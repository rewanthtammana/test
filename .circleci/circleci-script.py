...
def main():
    api_token = os.getenv('CIRCLE_CI_API_TOKEN')
    hmac_key = os.getenv('HMAC_SECRET_KEY')

    vcs_type = 'github'
    username = os.getenv('CIRCLE_PROJECT_USERNAME').strip()
    reponame = os.getenv('CIRCLE_PROJECT_REPONAME').strip()
    ...
    for item in env_vars.get('items', []):
        ...
        hmac_projects[hmac_value].append(f"circleci - {username}/{reponame} - {env_var_name}")

    # Output the results as JSON
    with open('circleci_results.json', 'w') as file:
